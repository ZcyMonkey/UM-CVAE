...
class Encoder_GCNTCNTCN_old(nn.Module):
...
class Decoder_GCNTCNTCN_old(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_frames, num_classes, translation, pose_rep, glob, glob_rot,
        ...
        self.tcn_decoding.append(CNN_layer(1,15,[3,3],0))
        self.tcn_decoding.append(CNN_layer(15,30,[3,3],0))
        self.tcn_decoding.append(CNN_layer(30,self.num_frames,[3,3],0))
        for i in range(self.tcn_layer-3):
    ...
    def forward(self, batch):
        ...
        z = self.feature_embedding(z)
        z = z[:,None,:].reshape(bs,1,njoints, nfeats)
        for i in range(self.tcn_layer-3):
            z = self.activation_gelu[i](self.tcn_decoding[i](z))
        ...
        for i in range(3,self.tcn_layer):
