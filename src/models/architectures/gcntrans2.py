...
import math
from .transformer import Decoder_TRANSFORMER as Decoder_GCNTRANS2
...
class Encoder_GCNTRANS2(nn.Module):
    ...
    def forward(self, batch):
        ...
        x = x.permute((0, 2, 3, 1))
        x = self.tcn(x)
        x = self.prelu(x).reshape(bs,1,self.njoints*self.node_feats_dim)
