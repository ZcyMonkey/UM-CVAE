...
def viz_epoch(model, dataset, epoch, params, folder, writer=None):
    ...
    random_classes = torch.randperm(num_classes)
    for num in range(num_classes):
        classes = random_classes[num:num+1]
        ...
        a = dataset.label_to_action_name(classes.numpy()[0])
        finalpath = os.path.join(folder, figname + str(dataset.label_to_action_name(classes.numpy()[0]))+".gif")
        tmp_path = os.path.join(folder, str(dataset.label_to_action_name(classes.numpy()[0]))+f"_subfigures_{figname}")
