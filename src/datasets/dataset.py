...
from ..utils.tensors import collate,collate_for_all
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, num_frames=1, sampling="conseq", sampling_step=1, split="train",
                 pose_rep="rot6d", translation=False, glob=False, max_len=-1, min_len=-1, num_seq_max=-1, **kwargs):
    ...
    def get_label_sample(self, label, n=1, return_labels=False, return_index=False):
        ...
        if n == 1:
            ...
            n =len(choices)
            data_index = choices
            x = torch.from_numpy(np.stack([self._get_item_data_index(index[di])[0] for di in data_index]))
    ...
    def get_label_sample_all(self, labels):
        samples = [self.get_label_sample(label, n=-1, return_labels=True, return_index=False) for label in labels]
        batch = collate_for_all(samples)
        x = batch["x"].squeeze(0)
        y = batch["y"]
        mask = batch["mask"]
        lengths = mask.sum(1)
